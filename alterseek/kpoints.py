...
class KPointsModifier:
    ...
    def convert_custom_path_from_input_basis(self, centroid_result) -> None:
        ...
        hand when writing one. Submitted-cell analysis reuses fractional
        coordinates directly, while legacy callers may still require a
        Cartesian-preserving conversion into SeeK-path's internal basis.
        """
        if not self.kpoints_data:
            raise ValueError("No custom KPOINTS path has been loaded.")

        if centroid_result.get(
        ...
        if centroid_result is not None:
            ...
            if c is None:
                ...
                if centroid_result.get(
                    ...
                    print(
                        "IBZ centroid (SeeK-path fractions reused in "
                ...
                try:
                    with open(os.path.join(OUTPUT_DIR, "spin_operations.txt"),
                        ...
                        centroid_basis_label = (
                            "SeeK-path fractions reused in submitted-cell basis"
                            if centroid_result.get(
                                'reuse_seekpath_fractions_in_submitted_basis',
                                False,
                            )
