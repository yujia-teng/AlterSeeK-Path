    ...
    def interactive_modify(self):
                ...
                if not path_choice:
                    ...
                    if 'ibz_kpath' in centroid_result and 'ibz_kpoints_frac' in centroid_result:
                        ...
                        ibz_coords = centroid_result.get(
                            'band_kpoints_frac',
                            centroid_result.get('path_kpoints_frac',
                            centroid_result['ibz_kpoints_frac']
                            )
                        )
                        auto_path = centroid_result.get('band_kpath', centroid_result['ibz_kpath'])
        ...
        try:
            ...
            if new_kpoints:
                ...
                if self.write_kpoints_file(new_kpoints, output_file, R_for_kpts):
                    if centroid_result is not None:
                        write_bandplot_lattice_config(
                            centroid_result.get('lattice_key', centroid_result.get('sc_type'))
                        )
