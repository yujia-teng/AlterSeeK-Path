...
def test_determinant_one_211_basis_is_the_native_output_cell(
    ...
    assert "Conventional/supercell BZ:    SG Pmm2 (25)" in stdout
...
def test_true_221_supercell_gets_native_hexagonal_labels_and_basis(
    ...
    assert "Conventional/supercell BZ:    SG P6mm (183)" in stdout
...
def test_211_converts_seekpath_points_to_submitted_basis_preserving_cartesian(
    ...
    # The oP1 helper supplies Y=(0,1/2,0) internally.  Its submitted-basis
    ...
    expected_internal_fraction = np.array([0.0, 0.5, 0.0])
    expected_output_fraction = np.array([0.5, 0.0, 0.0])
    assert y_rows
